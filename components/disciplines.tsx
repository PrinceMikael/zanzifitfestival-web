        ...
        <RevealGroup className="mt-14 flex flex-col gap-6">
          {DISCIPLINES.map((d) => (
            <RevealItem key={d.title}>
              <Link
                href={d.href}
                className="group block overflow-hidden rounded-lg border border-border bg-surface-dark-soft transition-colors hover:border-amber/50"
              >
                <div className="relative aspect-21/9 overflow-hidden sm:aspect-32/9">
                  <Image
                    src={d.image}
                    alt={d.title}
                    fill
                ...
                <div
                  className={cn(
                    'flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8',
