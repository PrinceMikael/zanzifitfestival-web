    ...
    >
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:h-24 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/register"
            className="group hidden items-center gap-2.5 whitespace-nowrap rounded-sm border border-amber/60 px-5 py-2.5 font-nav text-sm font-semibold tracking-wide text-white transition-all hover:-translate-y-0.5 hover:border-amber hover:shadow-[0_8px_24px_-8px_rgba(242,169,68,0.7)] sm:inline-flex"
          >
            Join Waitlist
          ...
          <nav className="mx-auto flex max-w-7xl flex-col px-4 pt-6 pb-4 sm:px-6" aria-label="Mobile">
            {MOBILE_NAV.map((item) => (
              <div key={item.href} className="border-b border-border/60">
                <Link
                  href={item.href}
                  className="flex items-center justify-between py-3 font-nav text-sm font-medium tracking-wide text-white"
                >
                  {item.label}
                  ...
                  <div className="flex flex-col pb-3 pl-4">
                    {item.children.map((child) => (
                      <Link
                        key={child.href}
                        href={child.href}
                        className="flex items-center justify-between py-2 font-nav text-sm font-medium tracking-wide text-white"
                      >
                        {child.label}
            ...
            <Link
              href="/register"
              className="mt-4 inline-flex items-center justify-center gap-2 rounded-sm bg-amber px-5 py-3.5 font-nav text-sm font-semibold tracking-wide text-primary-foreground"
            >
              Join the Waitlist
...
function NavLink({ href, active, children }: { href: string; active: boolean; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className={cn(
        'relative flex items-center gap-1.5 whitespace-nowrap font-nav text-sm font-medium tracking-wide transition-colors',
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        aria-haspopup="menu"
        className={cn(
          'flex items-center gap-1.5 whitespace-nowrap font-nav text-sm font-medium tracking-wide transition-colors',
          active ? 'text-amber' : 'text-white hover:text-amber',
        )}
      >
        {label}
            ...
            {overviewHref && (
              <>
                <Link
                  ...
                  role="menuitem"
                  className={cn(
                    'flex items-center justify-between rounded-sm px-3 py-2.5 font-nav text-sm font-medium tracking-wide transition-colors',
